...
int main(int argc, char *argv[])
{
  ...
  // field 0
  rawFieldName[0] = argv[4];
  degreeField[0] = atoi(argv[5]);
  lengthRatio[0] = atof(argv[6]);
  roundSeams[0] = string_to_bool(argv[7]);
  integralSeamless[0] = string_to_bool(argv[8]);
  exportMesh[0] = string_to_bool(argv[9]);
  // field 1
  rawFieldName[1] = argv[10];
  degreeField[1] = atoi(argv[11]);
  lengthRatio[1] = atof(argv[12]);
  roundSeams[1] = string_to_bool(argv[13]);
  integralSeamless[1] = string_to_bool(argv[14]);
  exportMesh[1] = string_to_bool(argv[15]);
  ...
  std::string PATH_DATA = "/Users/arpj/princeton/phd/papers/as_psf/experiments/structures/";
  ...
  igl::readOFF(PATH_OFF + structure_name + ".off", VMeshWhole, FMeshWhole);
  ...
  for (int i=0; i<NUM_N; i++){
    ...
    std::string mesh_generated_name = MESH_NAME +"_"+rawFieldName[i]+"_lr"+str_lr+"_sround"+str_rs+"_sintegral"+str_is+".off";

    // load in raw field
    std::cout<<"Reading N-RoSy field #"<<i<<" from "<<rawFieldName[i]<<"_"<<str_df<<"rosy.rawfield"<<std::endl;
    directional::read_raw_field(PATH_RAWFIELD + rawFieldName[i] + "_" + str_df + "rosy.rawfield", N[i], rawField[i]);
